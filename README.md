# LinkedIn Request Filter

A Chrome extension that automatically accepts (or ignores) LinkedIn connection requests based on your own filters.

Built by a non-technical founder in an afternoon using Claude. First thing I've ever shipped. Be nice.

## What it does
- Accepts requests from people with specific keywords in their bio (founder, VC, Stanford, SRCC etc.)
- Filters by minimum number of mutual connections
- One-click button to ignore everything that doesn't match your filters

## How to install

1. Download this repository by clicking the green **Code** button above, then **Download ZIP**
2. Unzip the folder on your computer
3. Open Chrome and go to `chrome://extensions`
4. Turn on **Developer mode** (top right corner)
5. Click **Load unpacked** and select the unzipped folder
6. Go to your LinkedIn invitations page: `linkedin.com/mynetwork/invitation-manager/received/`
7. Click the extension icon in your Chrome toolbar and hit **Accept Matching Requests**

## Tips
- Click the **Mutual Connections** filter on LinkedIn first to narrow down your list
- Run the extension, scroll down, and run it again until you've gone through everyone
- Edit the keywords directly in the popup before running

## Issues?
DM me on LinkedIn. I'll try to fix it. (I'm the whole support team.)
